...
<#
.SYNOPSIS
    Apply the terminal-theme Titanium palette to Windows Terminal in one command.

.DESCRIPTION
    Patches Windows Terminal settings.json with the Oh My Pi Titanium color
    scheme, JetBrainsMono Nerd Font Mono, and terminal-theme cursor / padding.
...
.PARAMETER Scheme
    Color scheme name to apply. Default: "Oh My Pi Titanium".
...
.PARAMETER OhMyPoshTheme
    oh-my-posh theme to use in $PROFILE. Default: "jandedobbeleer".
...
param(
    [string] $Scheme = 'Oh My Pi Titanium',
    ...
    [switch] $InstallFont,
    [string] $OhMyPoshTheme = 'jandedobbeleer',
    [switch] $SkipProfile
)
...
# Oh My Pi Titanium -- full 16-color ANSI palette + background / foreground / cursor.
$OhMyPiTitanium = [ordered]@{
    name                = 'Oh My Pi Titanium'
    background          = '#151820'
    foreground          = '#E8ECF4'
    cursorColor         = '#00B4FF'
    selectionBackground = '#0082B3'
    black               = '#0F1216'
    red                 = '#FF4757'
    green               = '#00FF88'
    yellow              = '#FFB347'
...
function Merge-Actions {
    ...
    $kept = @($Settings['actions'] | Where-Object {
        ...
        $ownedInstallerId = $entryId -and (
            $entryId.StartsWith('User.terminal-theme.') -or
            $entryId.StartsWith('User.wezterm-port.')
        )
        -not ($ownedInstallerId -or
              ($entryId -and ($newIds -contains $entryId)) -or
...
"@
    ...
    $replaceStart = $start
    $replaceEnd = $end
    if (-not $content.Contains($start) -and $content.Contains($legacyStart) -and $content.Contains($legacyEnd)) {
        $replaceStart = $legacyStart
        $replaceEnd = $legacyEnd
    }

    if ($content.Contains($replaceStart) -and $content.Contains($replaceEnd)) {
        $before = $content.Substring(0, $content.IndexOf($replaceStart))
        $afterIdx = $content.IndexOf($replaceEnd) + $replaceEnd.Length
        ...
        Write-Host "  refreshed terminal-theme block in $PROFILE" -ForegroundColor DarkGray
    }
    else {
        $append = ($content.TrimEnd() + "`r`n`r`n" + $block + "`r`n")
        Set-Content -LiteralPath $PROFILE -Value $append -NoNewline:$false
        Write-Host "  appended terminal-theme block to $PROFILE" -ForegroundColor DarkGray
    }
}

# --- main -------------------------------------------------------------------

Write-Host "terminal-theme" -ForegroundColor Magenta
...
if ($Scheme -eq 'Oh My Pi Titanium') {
    Merge-Scheme -Settings $settings -NewScheme $OhMyPiTitanium
}
...
Merge-Actions -Settings $settings -NewActions @(
    ...
    [ordered]@{
        id      = 'User.terminal-theme.paste'
        name    = 'Paste (terminal-theme)'
        command = 'paste'
    },
    [ordered]@{
        id   = 'User.terminal-theme.paste'
